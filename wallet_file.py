...
# get address file
work_file = Path.cwd() / "address.txt"
if Path(work_file).exists():
    if work_file.stat().st_size == 0:
        print("address file is None")
        time.sleep(5)
        sys.exit(1)
else:
    print("address file is not exist! in current path ")
    time.sleep(5)
    sys.exit(1)
...
def getWallet():
    ...
    while True:
        ...
        try:
            with open("address.txt","r",encoding="utf-8") as fp:
                for address in fp:
                    ...
                    for i in range(0, len(pageRet[1])):
                        getCount = getCount + 1
                        print("Testing: FileAddr:{} GenAddr:{}|Tested:{}".format(address, pageRet[1][i],int(getCount)), end="\r")
                        if address == pageRet[1][i]:
                    ...
                    success_msg = str(isCompress) + " " + "key:" + " " + key + " " + "address:" + address
